...
impl<T> typemap::Key for DieselMiddleware<T>
    where T: Connection + Send + Any
{
...
impl<T> BeforeMiddleware for DieselMiddleware<T>
    where T: Connection + Send + Any
{
...
impl<'a, 'b, T> DieselReqExt<T> for Request<'a, 'b>
    where T: Connection + Send + Any
{
    fn db_conn(&self) -> r2d2::PooledConnection<r2d2_diesel::ConnectionManager<T>> {
